...
def render_home(df,models):
    r=df.iloc[0]; storm=fnum(r["score_orage"]); ico=icon_for(r,storm); desc="Brume / brouillard possible" if fog_possible(r) else r["temps"]
    st.markdown(f'''<div class="hero"><div style="font-size:34px;font-weight:950;">Frasnes / Rèves</div><div style="opacity:.82;margin-bottom:15px;">Mobile multi-modèles • {pd.to_datetime(r["time"]).strftime("%d/%m %H:%M")}</div><div style="display:flex;justify-content:space-between;align-items:center;"><div><div style="font-size:68px;font-weight:950;line-height:.92;">{fnum(r["temperature_2m"]):.1f}°C</div><div style="font-size:21px;">Ressenti {fnum(r["apparent_temperature"]):.1f}°C</div><div style="font-size:17px;margin-top:10px;">{desc}</div></div><div style="font-size:78px;">{ico}</div></div></div>''',unsafe_allow_html=True)
...
def today_periods(df):
    st.markdown("### Aujourd’hui"); today=pd.Timestamp.now(tz="Europe/Brussels").tz_localize(None).date()
...
st.sidebar.title("Modèles météo")
selected_models = st.sidebar.multiselect(
    "Choisir les modèles",
    list(MODELS.keys()),
    default=["Best match", "ECMWF IFS", "ICON-EU", "AROME France HD"],
)
...
try:
    ...
    tab_home,tab_week,tab_alerts,tab_radar,tab_details=st.tabs(["🏠 Jour","📅 Semaine","⚠️ Alertes","🌧️ Radar","📊 Détails"])
    with tab_home:
        render_home(df,model_names); today_summary(df); today_periods(df); st.caption("V12.4 — mobile synchronisée multi-modèles avec la logique PC.")
