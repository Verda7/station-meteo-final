...
def icon_for(row, storm=0):
    ...
    if wet_signal:
        if sunshine_h >= 0.20 or cloud < 78:
            return icon_img("eclaircies_pluie.png")
        return icon_img("pluie_faible.png")

    # Important : si le modèle annonce du soleil réel sur l’heure/la période,
    # on évite l’icône “très nuageux” trop pessimiste.
    if sunshine_h >= 0.65:
...
def period_icon_for_summary(df, date, start_h, end_h):
    ...
    if wet_signal:
        if sunshine >= 0.8 or cloud < 78:
            return icon_img("eclaircies_pluie.png", 28)
        return icon_img("pluie_faible.png", 28)

    # Nouvelle priorité soleil/éclaircies :
    # si la période contient déjà pas mal de soleil, on affiche au moins partiellement nuageux.
...
def graphs(df):
    ...
    fig_storm.update_layout(
        ...
        margin=dict(l=10, r=10, t=45, b=10),
        legend=dict(orientation="h"),
        yaxis=dict(tickvals=[0,0.25,0.5,0.75,1.0], ticktext=["0 min","15 min","30 min","45 min","60 min"], range=[0, 100]),
        xaxis=xaxis_days,
    )
