...
def icon_img(filename, size=42):
    try:
        b64 = _icon_b64(filename)
        return f"<img src='data:image/png;base64,{b64}' style='width:{size}px;height:{size}px;object-fit:contain;vertical-align:middle;'>"
    except Exception:
        return "☁️"
...
def daily_icon_from_summary(r):
    ...
    if sunshine_h >= 12 and cloud < 25:
        return icon_img("clear_day.png")
    if sunshine_h >= 9 and cloud < 50:
        return icon_img("few_clouds.png")
    if sunshine_h >= 5 and cloud < 78:
        return icon_img("partly_cloudy.png")
    if cloud >= 78 or sunshine_h < 4:
        return icon_img("cloudy.png")
    return icon_img("partly_cloudy.png")
...
.week-row {
    display:grid;
    grid-template-columns: 44px 32px 56px 48px 48px 50px 48px;
    align-items:center;
    gap:4px;
    font-size:13px;
}
