...
def icon_for(row, storm=0):
    ...
    # Différence pluie :
    # bruine / pluie faible / pluie modérée / pluie forte
    if c in [51, 53, 55]:
    ...
    if rain > 0.1 or c in [61, 80]:
        return icon_img("pluie_faible.png")

    # Différence nuages :
    # ciel clair / peu nuageux / partiellement nuageux / très nuageux
    if c == 0 or cloud < 18:
...
.week-row {
    display:grid;
    grid-template-columns: 44px 76px 56px 48px 48px 50px 48px;
    align-items:center;
    gap:4px;
    font-size:13px;
}
...
<div class="week-compact" style="border-left:5px solid {color};">
  <div class="week-row">
    ...
    <div class="week-icon" style="gap:3px;">
