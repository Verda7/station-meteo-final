...
def icon_for(row, storm=0):
    ...
    if (rain > 0.1 or c in [61,63,80,81]) and (sunshine_h >= 0.25 or cloud < 72):
...
def period_icon_for_summary(df, date, start_h, end_h):
    ...
    # Important : s'il pleut mais qu'il y a aussi des éclaircies,
    # on affiche soleil + nuage + pluie, pas un simple nuage pluvieux.
    if rain >= 0.2 and sunshine >= 1.0:
