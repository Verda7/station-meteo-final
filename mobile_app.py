...
def icon_for(row, storm=0):
    ...
    # Pluie forte : icône pluie marquée.
    if rain>=3.0 or c in [65,82]: return "🌧️🌧️"
    # Pluie modérée : icône pluie normale.
    if rain>=1.0 or c in [63,81]: return "🌧️"
    # Pluie faible : nuage avec une seule goutte dessous.
    if rain>0.1 or c in [51,53,55,61,80]: return "<span style='display:inline-block;line-height:0.85;text-align:center;'>☁️<br><span style='font-size:0.72em;'>💧</span></span>"
...
""", unsafe_allow_html=True)
    ...
    st.caption("Format compact : min/max, soleil, pluie cumulée, rafales max et code couleur du jour.")
