...
def icon_for(row, storm=0):
    ...
    # Pluie faible visible même si le cumul est petit mais la proba est forte.
    # Si éclaircies/soleil dans la période, on affiche soleil + nuage + pluie.
    if wet_signal:
        if sunshine_h >= 0.25 or cloud < 72:
...
def period_icon_for_summary(df, date, start_h, end_h):
    ...
    # Pluie faible visible dans le résumé, même avec petit cumul.
    if wet_signal:
        if sunshine >= 1.0 or cloud < 72:
