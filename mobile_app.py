    ...
    for name, code in [("ECMWF IFS", MODELS.get("ECMWF IFS")), ("GFS Global", MODELS.get("GFS Global"))]:
        try:
            ...
            if d.empty or "time" not in d or "temperature_2m" not in d:
                erreurs.append(f"{name} : pas de données")
                continue

            d["time"] = pd.to_datetime(d["time"])
            d["temperature_2m"] = pd.to_numeric(d["temperature_2m"], errors="coerce")
            d = d[["time", "temperature_2m"]].rename(columns={"temperature_2m": name})
            frames.append(d)
        except Exception as e:
            erreurs.append(f"{name} : {str(e)[:100]}")

    if not frames:
        raise RuntimeError("Aucune donnée 15 jours disponible pour ECMWF/GFS")

    out = frames[0]
    for f in frames[1:]:
        out = out.merge(f, on="time", how="outer")

    cols = [c for c in ["ECMWF IFS", "GFS Global"] if c in out.columns]
    out["Moyenne"] = out[cols].mean(axis=1)
    out = out.sort_values("time").dropna(how="all", subset=cols + ["Moyenne"])
    return out, erreurs
...
def graphs(df):
    ...
    if horizon == "15 jours ECMWF/GFS":
        try:
            df15, erreurs15 = load_15j_ecmwf_gfs()
            ...
            st.caption("Courbes 15 jours : ECMWF IFS, GFS Global et moyenne des deux modèles.")
