...
def summarize_period(part, period_name=""):
    if part.empty:
        return None

    mid = part.iloc[len(part) // 2]

    temp_min = fnum(part["temperature_2m"].min())
    temp_max = fnum(part["temperature_2m"].max())
    temp_mean = fnum(part["temperature_2m"].mean())

    feel_min = fnum(part["apparent_temperature"].min())
    feel_max = fnum(part["apparent_temperature"].max())
    feel_mean = fnum(part["apparent_temperature"].mean())

    # Affichage façon app météo :
    # - après-midi : température max réelle
    # - nuit : température min réelle
    # - matin/soirée : température dominante, avec min/max disponibles
    if "Après-midi" in period_name:
        temp_display = temp_max
        feel_display = feel_max
        temp_label = "max"
    elif "Nuit" in period_name:
        temp_display = temp_min
        feel_display = feel_min
        temp_label = "min"
    else:
        temp_display = temp_mean
        feel_display = feel_mean
        temp_label = "moy"
...
<div class="period">
  ...
  <div style="margin-top:8px;font-size:14px;">
    🌡️ {s["temp_label"]} {s["temp"]:.0f}°C · ressenti {s["feel"]:.0f}°C<br>
    ↕️ min/max {s["temp_min"]:.0f}/{s["temp_max"]:.0f}°C<br>
...
def render_today_periods(df):
    ...
    for name, h1, h2 in PERIODES_JOUR:
        ...
        render_period_card(name, summarize_period(part, name))
    ...
    render_period_card("🌙 Nuit", summarize_period(night_part, "🌙 Nuit"), night_label)
...
def render_week_periods(df):
    ...
    for date in dates:
        ...
        day_min = fnum(day["temperature_2m"].min())
        day_max = fnum(day["temperature_2m"].max())
        feel_min = fnum(day["apparent_temperature"].min())
        feel_max = fnum(day["apparent_temperature"].max())
        day_rain = fnum(day["precipitation"].sum())
        day_gust = fnum(day["wind_gusts_10m"].max())
        day_storm = fnum(day["score_orage"].max())
        day_code, day_color, day_label = code_level(max(day_storm, day_gust - 25, day_rain * 4))

        st.markdown(f"#### {fr_day(date)} {pd.to_datetime(date).strftime('%d/%m')}")

        st.markdown(f"""
<div class="soft" style="border-color:{day_color};">
  <div style="display:flex;align-items:center;justify-content:space-between;">
    <div>
      <b>Résumé du jour</b><br>
      <span class="small">{day_label}</span>
    </div>
    <div style="font-size:24px;font-weight:950;">{day_min:.0f}/{day_max:.0f}°C</div>
    <span class="pill" style="background:{day_color};">{day_code}</span>
  </div>
  <div style="margin-top:8px;font-size:13px;">
    Ressenti {feel_min:.0f}/{feel_max:.0f}°C · 💧 {day_rain:.1f} mm · 💨 rafales {day_gust:.0f} km/h · ⛈️ {day_storm:.0f}/100
  </div>
</div>
""", unsafe_allow_html=True)
...
try:
    ...
    with tab_home:
        ...
        today_date = pd.Timestamp.now(tz="Europe/Brussels").tz_localize(None).date()
        d_today = df[pd.to_datetime(df["time"]).dt.date == today_date].copy()
        if not d_today.empty:
            tmin = fnum(d_today["temperature_2m"].min())
            tmax = fnum(d_today["temperature_2m"].max())
            fmin = fnum(d_today["apparent_temperature"].min())
            fmax = fnum(d_today["apparent_temperature"].max())
            rain = fnum(d_today["precipitation"].sum())
            gust = fnum(d_today["wind_gusts_10m"].max())
            storm = fnum(d_today["score_orage"].max())
            st.markdown(f"""
<div class="soft">
  <b>Résumé aujourd’hui</b><br>
  🌡️ min/max {tmin:.0f}/{tmax:.0f}°C · ressenti {fmin:.0f}/{fmax:.0f}°C<br>
  💧 {rain:.1f} mm · 💨 rafales {gust:.0f} km/h · ⛈️ {storm:.0f}/100
</div>
""", unsafe_allow_html=True)
