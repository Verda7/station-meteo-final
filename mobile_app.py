...
def load_15j_ecmwf_gfs():
    frames = []
    erreurs = []

    for name, code in [("ECMWF IFS", MODELS.get("ECMWF IFS")), ("GFS Global", MODELS.get("GFS Global"))]:
        if not code:
            erreurs.append(f"{name} : code modèle introuvable")
            continue
        ...
        try:
            ...
            r.raise_for_status()
            d = pd.DataFrame(r.json().get("hourly", {}))

            if d.empty or "time" not in d or "temperature_2m" not in d:
                erreurs.append(f"{name} : pas de données 15 jours")
                continue

            d["time"] = pd.to_datetime(d["time"])
            d["temperature_2m"] = pd.to_numeric(d["temperature_2m"], errors="coerce")
            d = d[["time", "temperature_2m"]].dropna()

            if d.empty:
                erreurs.append(f"{name} : données vides après nettoyage")
                continue

            d = d.rename(columns={"temperature_2m": name})
            frames.append(d)

        except Exception as e:
            erreurs.append(f"{name} indisponible : {str(e)[:140]}")

    if not frames:
        raise RuntimeError("Aucune donnée 15 jours disponible")

    out = frames[0]
    for f in frames[1:]:
        out = out.merge(f, on="time", how="outer")

    model_cols = [c for c in ["ECMWF IFS", "GFS Global"] if c in out.columns]

    if len(model_cols) == 2:
        common = out[model_cols].dropna()
        if not common.empty and common["ECMWF IFS"].round(2).equals(common["GFS Global"].round(2)):
            out["Moyenne ECMWF/GFS"] = pd.NA
            erreurs.append("ECMWF et GFS semblent identiques : moyenne masquée pour éviter une fausse courbe.")
        else:
            out["Moyenne ECMWF/GFS"] = out[model_cols].mean(axis=1)
    else:
        out["Moyenne ECMWF/GFS"] = pd.NA
        erreurs.append("Moyenne non calculée : il faut ECMWF + GFS disponibles.")

    out = out.sort_values("time")
    return out, erreurs, model_cols
...
def graphs(df):
    ...
    horizon = st.radio(
        "Horizon",
        ["48h", "7 jours", "15 jours ECMWF/GFS"],
        horizontal=True,
        index=0
    )

    if horizon == "15 jours ECMWF/GFS":
        try:
            df15, erreurs15, model_cols15 = load_15j_ecmwf_gfs()
            ...
            if "Moyenne ECMWF/GFS" in df15.columns and df15["Moyenne ECMWF/GFS"].notna().any():
                fig_15.add_trace(go.Scatter(
                    x=df15["time"],
                    y=df15["Moyenne ECMWF/GFS"],
                    mode="lines",
                    name="Moyenne ECMWF/GFS",
                    line=dict(color="#22c55e", width=5)
                ))

            fig_15.update_layout(
                title="Prévision 15 jours — température ECMWF / GFS / moyenne",
                height=360,
            ...
            if len(model_cols15) < 2:
                st.warning("ECMWF ou GFS indisponible : la moyenne 15 jours n’est pas affichée.")
            st.caption("Courbes 15 jours : seules les données réellement disponibles sont affichées. La moyenne apparaît uniquement si ECMWF + GFS sont disponibles.")

            if erreurs15:
                with st.expander("Détails modèles 15 jours", expanded=False):
                    for e in erreurs15:
                        st.write("• " + str(e))

        except Exception as e:
            st.error("Impossible de charger le graphique 15 jours ECMWF/GFS.")
            st.code(str(e))
