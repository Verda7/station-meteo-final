...
HOURLY = ["temperature_2m","apparent_temperature","precipitation","precipitation_probability","weather_code","cloud_cover","wind_speed_10m","wind_gusts_10m","wind_direction_10m","relative_humidity_2m","dew_point_2m","pressure_msl","uv_index","cape","freezing_level_height"]
...
def icon_for(row, storm=0):
    ...
    if fog_possible(row): return "🌫️"
    if fnum(storm)>=82 or c in [95,96,99]: return "⛈️"
    if rain>=2.5 or c in [63,65,81,82]: return "🌧️"
    if rain>0.2 or c in [51,53,55,61,80]: return "🌦️"
    if cloud>=80 or c==3: return "☁️"
    if cloud>=35 or c in [1,2]: return "⛅"
    return "☀️"
...
def summary_7_days(df):
    ...
    for _, r in day.iterrows():
        ...
        ico = "🌫️" if int(fnum(r["fog"])) else icon_for(fake, r["storm"])

        if fnum(r["rain"]) >= 5:
            trend = "Pluie à surveiller"
        elif fnum(r["gust"]) >= 60:
            trend = "Vent soutenu"
        elif fnum(r["storm"]) >= 50:
            trend = "Risque orageux"
        elif int(fnum(r["fog"])):
            trend = "Brume/brouillard possible"
        else:
            trend = "Temps calme"

        st.markdown(f"""
<div class="soft" style="border-left:6px solid {color};">
  <div style="display:flex;align-items:center;justify-content:space-between;gap:10px;">
    <div style="min-width:78px;">
      <b>{fr_day(r["date"])}</b><br>
      <span class="small">{pd.to_datetime(r["date"]).strftime("%d/%m")}</span>
    </div>
    <div style="font-size:38px;">{ico}</div>
    <div style="font-size:24px;font-weight:950;">{fnum(r["temp_min"]):.0f}/{fnum(r["temp_max"]):.0f}°C</div>
    <span class="pill" style="background:{color};">{code}</span>
  </div>

  <div style="margin-top:10px;font-size:13px;">
    Ressenti {fnum(r["feel_min"]):.0f}/{fnum(r["feel_max"]):.0f}°C · 💧 {fnum(r["rain"]):.1f} mm / {fnum(r["rain_proba"]):.0f}%<br>
    💨 vent {fnum(r["wind"]):.0f} km/h · rafales {fnum(r["gust"]):.0f} km/h · ⛈️ {fnum(r["storm"]):.0f}/100<br>
    <b>{trend}</b> · {label}
  </div>
</div>
...
try:
    ...
    tab_home,tab_summary,tab_week,tab_alerts,tab_graphs,tab_radar,tab_details=st.tabs(["🏠 Jour","🗓️ Résumé 7j","📅 Semaine","⚠️ Alertes","📈 Graphiques","🌧️ Radar","📊 Détails"])
    with tab_home:
        render_home(df,model_names); today_summary(df); today_periods(df); st.caption("V12.8 — graphes colorés + résumé 7 jours.")
