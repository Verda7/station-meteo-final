...
def icon_for(row, storm=0):
    ...
    if rain>=3.0 or c in [65,82]: return "🌧️🌧️"
    if rain>=1.0 or c in [63,81]: return "🌧️"
    if rain>0.1 or c in [51,53,55,61,80]: return "<span style='display:inline-block;line-height:0.72;text-align:center;'>☁️<br><span style='font-size:0.42em;letter-spacing:-1px;'>╱╱</span></span>"

    # Différence claire :
    # ☀️ ciel clair
    # 🌤️ peu nuageux
    # ⛅ partiellement nuageux
    # ☁️ couvert/très nuageux
    if c == 0 or cloud < 18: return "☀️"
    if c == 1 or cloud < 38: return "🌤️"
    if c == 2 or cloud < 72: return "⛅"
    return "☁️"
...
def daily_icon_from_summary(r):
    ...
    if rain >= 3:
        return "🌧️"
    if rain >= 0.2:
        return "<span style='display:inline-block;line-height:0.72;text-align:center;'>☁️<br><span style='font-size:0.42em;letter-spacing:-1px;'>╱╱</span></span>"

    # Icône du résumé 7 jours selon soleil réel estimé + nébulosité :
    # ☀️ = très lumineux / ciel clair
    # 🌤️ = peu nuageux
    # ⛅ = partiellement nuageux
    # ☁️ = très nuageux/couvert
