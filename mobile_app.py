...
.week-row {
    display:grid;
    grid-template-columns: 44px 32px 62px 52px 56px 52px;
    align-items:center;
    gap:4px;
    font-size:13px;
}
...
""", unsafe_allow_html=True)
    ...
    st.caption("Format compact : min/max, pluie cumulée, rafales max et code couleur du jour.")
...
try:
    ...
    with tab_home:
        render_home(df,model_names); today_summary(df); today_periods(df); st.caption("V13 — résumé 7 jours compact + courbe ressenti orange.")
