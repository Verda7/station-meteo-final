...
def load_15j_ecmwf_gfs():
    ...
    for name, code in [("ECMWF IFS", MODELS.get("ECMWF IFS")), ("GFS Global", MODELS.get("GFS Global"))]:
        ...
        try:
            ...
            if d.empty:
                erreurs.append(f"{name} : données vides")
                continue
            ...
            erreurs.append(f"{name} indisponible : {str(e)[:120]}")
    ...
    if len(model_cols) == 2:
        ...
        if not common.empty and common["ECMWF IFS"].round(2).equals(common["GFS Global"].round(2)):
            ...
            erreurs.append("ECMWF et GFS sont identiques : moyenne masquée.")
        else:
            out["Moyenne ECMWF/GFS"] = out[model_cols].mean(axis=1)
    else:
        out["Moyenne ECMWF/GFS"] = pd.NA
        erreurs.append("Moyenne non affichée : il faut ECMWF + GFS disponibles.")
...
def alerts(df):
    ...
    score,storm,rain,hail,down,sup,tor,gust=risk_24h(df); c,col,lab=code_level(score, storm)
...
def graphs(df):
    ...
    if horizon == "15 jours ECMWF/GFS":
        try:
            ...
            st.caption("Courbes 15 jours : ECMWF IFS, GFS Global et moyenne uniquement si les deux modèles sont disponibles.")
