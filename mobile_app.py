...
def icon_for(row, storm=0):
    ...
    if fog_possible(row): return icon_img("fog.png")
    if fnum(storm)>=82 or c in [95,96,99]: return icon_img("thunderstorm.png")
    if rain>=3.0 or c in [65,82]: return icon_img("heavy_rain.png")
    if rain>=1.0 or c in [63,81]: return icon_img("rain.png")
    if c in [51,53,55]: return icon_img("drizzle.png")
    if rain>0.1 or c in [61,80]: return icon_img("light_rain.png")
    if c == 0 or cloud < 18: return icon_img("clear_day.png")
    if c == 1 or cloud < 38: return icon_img("few_clouds.png")
    if c == 2 or cloud < 72: return icon_img("partly_cloudy.png")
    return icon_img("cloudy.png")
...
def fetch_model(name, code):
    ...
    # Sécurise les colonnes qui peuvent manquer selon les modèles.
    for c in HOURLY:
...
def load_multimodel(selected_models):
    ...
    # Si l'utilisateur choisit "Prévision standard", on utilise l'API sans forcer de modèle.
    # C'est le mode le plus stable.
    for name in selected_models:
        try:
            code = MODELS.get(name)
            frames.append(fetch_model(name, code))
        except Exception as e:
            errors.append(f"{name}: {str(e)[:90]}")

    # Fallback automatique : même si tous les modèles forcés échouent,
    # l'app utilise la prévision standard et ne plante pas.
    if not frames:
        try:
            frames.append(fetch_model("Prévision standard", None))
            errors.append("Fallback automatique utilisé : Prévision standard Open-Meteo.")
        except Exception as e:
            raise RuntimeError("Aucun modèle disponible, même en prévision standard.")
    ...
    avg = add_scores(avg)
    model_names = sorted(all_df["modele"].dropna().unique().tolist())
...
def render_period_card(name,s,night=""):
    ...
    html=f'''<div class="period"><div style="display:flex;align-items:center;justify-content:space-between;"><div><b>{title}</b><br><span class="small">{s["label"]}{fog}</span></div><div style="font-size:35px;display:flex;align-items:center;justify-content:center;overflow:visible;">{s["icon"]}</div><span class="pill" style="background:{s["color"]};">{s["code"]}</span></div><div style="margin-top:8px;font-size:14px;">🌡️ {s["temp_label"]} {s["temp"]:.0f}°C · ressenti {s["feel"]:.0f}°C<br>↕️ min/max {s["temp_min"]:.0f}/{s["temp_max"]:.0f}°C<br>💧 {s["rain"]:.1f} mm · proba {s["rain_proba"]:.0f}%<br>💨 vent {s["wind"]:.0f} km/h · rafales {s["gust"]:.0f} km/h<br>⛈️ orage {s["storm"]:.0f}/100 · 🧊 grêle {s["hail"]:.0f}/100 · UV {s["uv"]:.1f}</div></div>'''
...
def daily_icon_from_summary(r):
    ...
    fog_day = int(fnum(r.get("fog", 0))) == 1

    if storm >= 70:
        return icon_img("thunderstorm.png")
    if rain >= 6:
        return icon_img("heavy_rain.png")
    if rain >= 1.2:
        return icon_img("rain.png")
    if rain >= 0.2:
        return icon_img("light_rain.png")

    # Brouillard seulement s'il est vraiment dominant.
    # S'il y a beaucoup de soleil prévu, on privilégie soleil/éclaircies.
    if fog_day and (sunshine_h < 5 or cloud >= 80):
        return icon_img("fog.png")

    if sunshine_h >= 12 and cloud < 35:
        return icon_img("clear_day.png")
    if sunshine_h >= 9 and cloud < 60:
        return icon_img("few_clouds.png")
    if sunshine_h >= 5 and cloud < 82:
        return icon_img("partly_cloudy.png")
    if cloud >= 82 or sunshine_h < 4:
        return icon_img("cloudy.png")
    return icon_img("partly_cloudy.png")
...
.week-row {
    display:grid;
    grid-template-columns: 44px 42px 56px 48px 48px 50px 48px;
    align-items:center;
    gap:4px;
    font-size:13px;
}
...
.week-icon {
    font-size:25px;display:flex;align-items:center;justify-content:center;overflow:visible;
    text-align:center;
}
.week-temp {
    font-size:17px;
...
<div class="week-compact" style="border-left:5px solid {color};">
  <div class="week-row">
    ...
    <div class="week-pill" style="background:{color};">{code}</div>
  </div>
</div>
""", unsafe_allow_html=True)
...
st.caption("Mode stable conseillé : Prévision standard. Tu peux aussi choisir plusieurs modèles pour faire une moyenne.")

MODELS = {
    "Prévision standard": None,
...
default_models = ["Prévision standard"]
