...
def render_interface_app(avg_table, days=3):
    if avg_table is None or avg_table.empty:
    ...
    needed_cols = [
        "time", "weather_code", "precipitation", "cloud_cover", "score_orage",
        "temperature_2m", "apparent_temperature", "wind_speed_10m",
        "wind_gusts_10m", "relative_humidity_2m", "score_pluie_intense",
        "risque_supercellule"
    ]
    ...
    now_row = d0.iloc[0]
    icon = visual_weather_icon(
        now_row.get("weather_code", 3),
        now_row.get("precipitation", 0),
        now_row.get("cloud_cover", 0),
        now_row.get("score_orage", 0),
    )

    risk = safe_float(now_row.get("score_orage", 0))
    risk_color = app_risk_color(risk)

    st.markdown("""
    <style>
    .app-card {
        background: linear-gradient(135deg,#174ea6 0%,#1d4ed8 45%,#0f172a 100%);
        border-radius:28px;
        padding:26px;
        color:white;
    ...
    .day-row {
        display:grid;
        grid-template-columns:90px 52px 110px 90px 90px 90px 1fr;
        gap:8px;
        align-items:center;
        background:#111827;
        border:1px solid #263244;
    ...
    @media (max-width: 800px) {
        .app-temp {font-size:52px;}
        .app-title {font-size:28px;}
        .day-row {
            grid-template-columns:70px 42px 90px 80px;
            font-size:13px;
        }
        .mini-card {
            padding:10px;
    ...
    <div class="app-card">
        ...
        <div style="display:grid;grid-template-columns:1.3fr .7fr;gap:20px;align-items:center;margin-top:18px;">
            <div>
                <div class="app-temp">{safe_float(now_row.get("temperature_2m")):.1f}°C</div>
                <div style="font-size:20px;margin-top:6px;">Ressenti {safe_float(now_row.get("apparent_temperature")):.1f}°C</div>
        ...
        <div style="display:grid;grid-template-columns:repeat(5,1fr);gap:10px;margin-top:22px;">
            <div class="mini-card"><b>Pluie</b><br>{safe_float(now_row.get("precipitation")):.1f} mm</div>
            <div class="mini-card"><b>Vent</b><br>{safe_float(now_row.get("wind_speed_10m")):.0f} km/h</div>
            <div class="mini-card"><b>Rafales</b><br>{safe_float(now_row.get("wind_gusts_10m")):.0f} km/h</div>
            <div class="mini-card"><b>Humidité</b><br>{safe_float(now_row.get("relative_humidity_2m")):.0f} %</div>
    ...
    max_storm = safe_float(d0.head(24)["score_orage"].max())
    max_gust = safe_float(d0.head(24)["wind_gusts_10m"].max())
    max_rain = safe_float(d0.head(24)["score_pluie_intense"].max())
    max_super = safe_float(d0.head(24)["risque_supercellule"].max())
    ...
    for _, r in daily.iterrows():
        ...
        <div class="day-row">
            ...
            <div><b>{safe_float(r["temp_min"]):.0f}° / {safe_float(r["temp_max"]):.0f}°C</b></div>
            <div>💧 {safe_float(r["rain"]):.1f} mm</div>
            <div>💨 {safe_float(r["gust"]):.0f} km/h</div>
            <div>⛈️ {safe_float(r["storm"]):.0f}/100</div>
